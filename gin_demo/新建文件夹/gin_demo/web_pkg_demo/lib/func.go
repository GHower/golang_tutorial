...
import (
	...
	"flag"
	"fmt"
	dlog "gin_demo/web_pkg_demo/common/log"
	"io/ioutil"
	"log"
	"math/rand"
	"net"
...
func InitModule(configPath string, modules []string) error {
	...
	if InArrayString("base", modules) {
		...
		// fixme: 获取开启的sql,可以升级成从配置中心获取，以及多数据源
		dbActive := GetStringConf("base.base.db_active")
		dbActiveList := strings.Split(dbActive, ",")
		// 多数据源遍历
		for _, dbConf := range dbActiveList {
			// 加载数据库配置并初始化实例
			fmt.Println(dbConf, modules)
			if InArrayString(dbConf, modules) {
				if err := InitDBPool(GetConfPath("db_" + dbConf)); err != nil {
