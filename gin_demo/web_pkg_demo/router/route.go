...
func InitRouter(middlewares ...gin.HandlerFunc) *gin.Engine {
	...
	v1 := router.Group("v1")
	v1.Use(middleware.LimitingApi(), middleware.RequestLog())
	{
		v1.GET("/login", loginTest)
	}
	// 通道模拟的令牌桶，类似放票的模式
	v2 := router.Group("v2")
	v2.Use(middleware.LimitingApiTokenBucket(), middleware.RequestLog())
	{
	...
	// 通道模拟的漏桶
	v3 := router.Group("v3")
	v3.Use(middleware.LimitingApiLeakyBucket(), middleware.RequestLog())
	{
