...
func InitDBPool(path string) error {
	// fixme: 这里偷个懒，先用两个数据库测试一下，后续改成通用的模式
	fmt.Printf("数据库配置文件::%v\n", path)
	if strings.Contains(path, "postgres") {
		return InitProgreSQLDBPool(path)
	}
	//InitProgreSQLDBPool(path)
	//普通的db方式
	...
	//手动配置连接,将mysql作为默认
	if dbpool, err := GetDBPool("mysql"); err == nil {
		DBDefaultPool = dbpool
	}
	if dbpool, err := GetGormPool("mysql"); err == nil {
